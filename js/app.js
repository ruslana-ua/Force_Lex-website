 ...
 */
    ...
    (() => {
        ...
        const baronElementsSw = document.querySelectorAll(".baron--sw");
        if (window.matchMedia("(max-width: 991.98px)").matches) baronElementsSw.forEach((function(element) {
            window.addEventListener("load", (function() {
                core({
                    direction: "h",
                    root: element,
                    scroller: ".baron__scroller--sw",
                    bar: ".baron__bar--sw"
                }).autoUpdate();
            }));
        }));
        ...
        let formValidate = {
            ...
            validateInput(formRequiredItem) {
                let error = 0;
                if (formRequiredItem.dataset.required === "name") {
            ...
            addError(formRequiredItem) {
                ...
                if (formRequiredItem.dataset.error) formRequiredItem.parentElement.insertAdjacentHTML("beforeend", `<div class="form__error"><div class="form__arrow"></div>${formRequiredItem.dataset.error}</div>`);
            },
        ...
        function formSubmit() {
            ...
            async function formSubmitAction(form, e) {
                ...
                if (error === 0) {
                    ...
                    e.preventDefault();
                    if (form.querySelector("._form-error") && form.hasAttribute("data-goto-error")) gotoblock_gotoBlock("#block-form", true, 1e3);
                }
            }
        ...
        function initSliders() {
            if (document.querySelector(".swiper")) {
                ...
                if (window.matchMedia("(min-width: 991.98px)").matches) new swiper_core_Swiper(".publications-block__slider", {
        ...
        if (window.innerWidth <= 991.98) {
            ...
            const selectUaRadio = document.getElementById("select-ua");
            const selectRuRadio = document.getElementById("select-ru");
            if (currentURL.includes("/ru")) {
                selectRuRadio.checked = true;
                selectUaRadio.checked = false;
            }
        }
        if (window.innerWidth > 991.98) {
        ...
        if (window.innerWidth > 767.98) {
            const tabTitles = document.querySelectorAll(".tabs-team__title");
            const countElement = document.querySelector(".team__count-1");
            function updateCount() {
                tabTitles.forEach(((title, index) => {
                    if (title.classList.contains("_active")) countElement.textContent = (index + 1).toString();
                }));
            }
            tabTitles.forEach(((title, index) => {
                title.addEventListener("click", (() => {
                    tabTitles.forEach((t => t.classList.remove("_active")));
        ...
        function simulateButtonClick() {
            ...
            if (!siteClosed) {
                var button = document.querySelector('a[data-popup="#popup"]');
        ...
        setTimeout((function() {
            performActions();
            recurringActionInterval = setInterval(performActions, 12e4);
        }), 6e4);
        ...
        var includeElement = document.getElementById("include");
        if (includeElement) document.documentElement.classList.add("include");
        const tabTitles = document.querySelectorAll(".tabs-about__title");
        const countElement = document.querySelector(".about-02__count-1");
        function updateCount() {
            tabTitles.forEach(((title, index) => {
                if (title.classList.contains("_active")) countElement.textContent = (index + 1).toString();
            }));
        ...
        updateCount();
        var pageTeam = document.querySelector(".page-team");
        if (pageTeam) document.documentElement.classList.add("team--st");
        const servicesTabs = document.querySelector(".services__tabs");
        document.querySelector(".tabs-services__title._tab-active");
        if (servicesTabs) {
            function handleMediaQuery(event) {
                if (event.matches) servicesTabs.removeAttribute("data-tabs"); else servicesTabs.setAttribute("data-tabs", "true");
            }
        ...
        function executeGotoBlock() {
            gotoblock_gotoBlock("._spoller-active", true, 1e3);
        }
        const container = document.querySelector(".spollers-services");
        if (container) {
            function handleMutation(mutationsList, observer) {
                mutationsList.forEach((mutation => {
                    if (mutation.type === "attributes" && mutation.attributeName === "class") if (mutation.target.classList.contains("_spoller-active")) setTimeout(executeGotoBlock, 500);
                }));
            }
        ...
        function movePublicationTopToParent() {
            if (window.innerWidth < 767.98) {
                const publicationItems = document.querySelectorAll(".publications__item");
                publicationItems.forEach((item => {
                    const topBlock = item.querySelector(".publications__top");
                    if (topBlock) item.insertBefore(topBlock, item.firstChild);
                }));
            }
        }
        window.addEventListener("load", movePublicationTopToParent);
        window.addEventListener("resize", movePublicationTopToParent);
        window["FLS"] = false;
        isWebp();
        menuInit();
        spollers();
